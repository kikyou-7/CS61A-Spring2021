...
def piggy_points(score):
    ...
    min_digit = sqr % 10
    while sqr > 0: 
        min_digit = min(min_digit , sqr%10)
        sqr //= 10
    return min_digit + 3
    # END PROBLEM 2
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0,socre1):
        cur_high = running_high
        cur_score = score0 if who==0 else socre1
        if cur_score - last_score > cur_high:
            cur_high=cur_score - last_score
            print('Player',who,'has reached a new maximum point gain.',cur_high,'point(s)!')
